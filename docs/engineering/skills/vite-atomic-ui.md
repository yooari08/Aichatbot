...
1. **`src/app/components/atoms`**: 단일 역할 UI
2. **`src/app/components/molecules`**: atoms 조합
3. **`src/app/components/organisms`**: 화면 단위 블록
4. **`src/app/pages`**: 라우트별 페이지 — 데이터·문구 주입과 조립만 두고 반복 UI는 위로 올림

기존 atom이 있으면 **새 마크업보다 확장·재사용**을 우선합니다.

## Atoms — 이 레포에서 쓰는 방식

- **Button**: `variant` (`primary` | `secondary` | `ghost` | `neutral` | `text`), 링크는 `as="a"` + `href`
- **Input**: `label` + `id`/`name`으로 연결. `value`/`onChange`로 제어 컴포넌트
- **CheckboxSet**: 제어 모드는 `checked` + `onChange` **둘 다 필수**. 비제어는 `defaultChecked`만. **동시 사용 금지**
- **Tag**: `variant` `stack` | `page` | `default` — 다크 배경 태그는 `stack`/`page`로 대비 유지
- **TitleBlock**: 공통 타이포는 `app-title` / `app-subtext`
...
- `strict` 유지. **`any` 지양**
- 의미 있는 변경 후 **`npm run typecheck`** 실행
...
- 공통 규칙 및 검수 체크리스트: `docs/engineering/harness-guardrails.md`
