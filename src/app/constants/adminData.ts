...
import type { KbDocument, AdminUser, NavSection, AdminView } from "../types/admin";
...
export const NAV: NavSection[] = [
  {
    label: "콘텐츠 관리",
    groups: [
      { key: "kb", label: "지식베이스", icon: BookOpen, subs: [{ key: "documents", label: "문서 목록" }, { key: "faq", label: "FAQ 플레이어" }, { key: "categories", label: "카테고리 설정" }, { key: "indexLog", label: "색인 로그" }] },
  ...
  },
  {
    label: "분석",
    groups: [
      { key: "analytics", label: "분석 대시보드", icon: BarChart2, subs: [{ key: "analytics", label: "사용 현황" }, { key: "quality", label: "품질 분석" }] },
    ],
  },
  {
    label: "시스템",
    groups: [
      { key: "users", label: "사용자 관리", icon: Users, subs: [{ key: "users", label: "사용자 목록" }, { key: "roles", label: "역할/권한" }, { key: "auditLog", label: "감사 로그" }] },
